		...
		<div class="form-group">
			...
			<select name="art_id" id="art_id" class="custom-select select2">
				...
				<?php 
					...
					$arts = $conn->query("SELECT a.*,u.name as aname FROM arts a inner join users u on u.id = a.artist_id where a.id not in (SELECT art_id FROM arts_fs ".(isset($id) ? " where id != $id " : "")." ) AND u.id = $login2_id");
...
<script>
   ...
   isValid = false;
		...
		if(isValid){
		$.ajax({
			url:'admin/ajax.php?action=save_art_fs',
