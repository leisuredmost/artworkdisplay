...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM users where username = '".$username."' and password = '$password' and user_type = 1  ");
		if($qry->num_rows > 0){
	...
	function login2(){
		if(isset($_POST['login2_id'])){
			$login2_id = $_POST['login2_id'];
		}
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM users where username = '".$username."' and password = '$password' and user_type = 2 ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
					$_SESSION['login2_'.$key] = $value;
			}
				return 1;
		}else{
			return 3;
	...
	function save_art2(){
		...
		if(empty($id)){
			...
			if($save){
				$id = $this->db->insert_id;
				$folder = "admin/assets/uploads/artist_".$id;
				if(is_dir($folder)){
					$files = scandir($folder);
					foreach($files as $k =>$v){
						if(!in_array($v, array('.','..'))){
	...
	function save_order(){
		...
		if(empty($id)){
			...
			if($save){
				...
				$data .= ", orders_id = '$id'";
				$this->db->query("INSERT INTO notifications set ".$data);
			}
		}else{
			$save = $this->db->query("UPDATE users set ".$data." where id=".$id);
		}
	...
	function save_rating_comment(){
		...
		if(empty($id)){
			...
			if($save){
				...
				$this->db->query("INSERT INTO ratings_comments set ".$data);
			}
			if($save){
				$id = $this->db->insert_id;
				$data = "message = 'New Ratings'";
				$data .= ", ratings_comments_id = '$id' ";
				$this->db->query("INSERT INTO notifications set ".$data);
			}
